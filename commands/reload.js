...
reload.default = async (ctx) => {
    ...
    if (response) return await ctx.message.reply(`Error Unloading: ${response}`);

    response = ctx.self.loadCommand(command.name);
    if (response) return await ctx.message.reply(`Error Loading: ${response}`);

    return await ctx.message.reply(`The command \`${command.name}\` has been reloaded`);
};
